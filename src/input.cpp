...
namespace network {
bool active = false;
uint8_t key_bitmap[32] = {0};
int32_t mouse_x = 0, mouse_y = 0;
int32_t screen_w = 0, screen_h = 0;
bool osu_foreground = false;
int32_t osu_left = 0, osu_top = 0, osu_right = 0, osu_bottom = 0;
} // namespace network
...
bool is_pressed(int key_code) {
    if (network::active) {
        return network::bitmap_has(key_code);
    }
    if (key_code == 16) {
...
std::pair<double, double> get_xy() {
    if (network::active) {
        ...
        int net_h = network::screen_w > 0 ? network::screen_w : horizontal;
        int net_v = network::screen_h > 0 ? network::screen_h : vertical;

        if (network::osu_foreground) {
            s_height = osu_y * 0.8;
            s_width = s_height * 4 / 3;
            if (!is_letterbox) {
                letter_x = network::osu_left + ((network::osu_right - network::osu_left) - s_width) / 2;
                letter_y = network::osu_top + osu_y * 0.117;
            } else {
                double l = (net_h - osu_x) * (osu_h + 100) / 200.0;
                double r = l + osu_x;
                letter_x = l + ((r - l) - s_width) / 2;
                letter_y = (net_v - osu_y) * (osu_v + 100) / 200.0 + osu_y * 0.117;
            }
        } else {
            s_width = net_h;
            s_height = net_v;
        ...
        if (!is_letterbox) {
            letter_x = floor(1.0 * network::mouse_x / osu_x) * osu_x;
            letter_y = floor(1.0 * network::mouse_y / osu_y) * osu_y;
        }
        double fx = (1.0 * network::mouse_x - letter_x) / s_width;
        if (is_left_handed) fx = 1 - fx;
        double fy = (1.0 * network::mouse_y - letter_y) / s_height;
        fx = std::min(fx, 1.0);
        fx = std::max(fx, 0.0);
        fy = std::min(fy, 1.0);
        fy = std::max(fy, 0.0);
    ...
    if (handle) {
        char w_title[256] = {0};
        GetWindowTextA(handle, w_title, sizeof(w_title));
